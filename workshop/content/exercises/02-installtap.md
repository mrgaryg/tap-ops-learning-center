...
```execute
sudo tanzu package repository add tanzu-tap-repository --url tapworkshopoperators.azurecr.io/tap15/tap-packages:1.5.0 --namespace tap-install
```
...
```execute
sudo tanzu package install tap -p tap.tanzu.vmware.com -v 1.5.0 --values-file $HOME/tap-values.yaml -n tap-install
```
